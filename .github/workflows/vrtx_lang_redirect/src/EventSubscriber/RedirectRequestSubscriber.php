...
namespace Drupal\vrtx_lang_redirect\EventSubscriber;
...
class RedirectRequestSubscriber implements EventSubscriberInterface {
  ...
  /**
   * VrtxTranslationRedirectRequestSubscriber constructor.
   *
   ...
   *   The currently active route match object.
   */
  public function __construct(ConfigFactoryInterface $config_factory, LanguageManagerInterface $language_manager, RouteMatchInterface $route_match) {
    $this->config = $config_factory->get('vrtx_lang_redirect.default');
    ...
    if ($entity && $entity->isTranslatable()) {
      ...
      if ($entity_language->getId() != $current_language->getId()) {
        ...
        if ($redirect_path != $current_path) {
          ...
          $response = new LocalRedirectResponse($redirect_url, 301);
